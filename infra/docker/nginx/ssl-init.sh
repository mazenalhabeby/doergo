...
#
# Usage: ./ssl-init.sh hbcfield.hbc-solution.io your@email.com
#
# Prerequisites:
...
echo "=== HBCField SSL Certificate Setup ==="
echo "Domain: $DOMAIN"
echo "Email:  $EMAIL"
echo ""
...
docker run -d --name hbcfield-ssl-init \
  -p 80:80 \
  -v "$COMPOSE_DIR/nginx/default.conf.tmp:/etc/nginx/conf.d/default.conf:ro" \
  -v "hbcfield-certbot-www:/var/www/certbot" \
  nginx:1.27-alpine

# Step 3: Obtain certificate
echo "→ Requesting certificate from Let's Encrypt..."
docker run --rm \
  -v "hbcfield-certbot-conf:/etc/letsencrypt" \
  -v "hbcfield-certbot-www:/var/www/certbot" \
...
docker stop hbcfield-ssl-init && docker rm hbcfield-ssl-init
...
echo "Certificates stored in Docker volume: hbcfield-certbot-conf"
echo ""
echo "Next steps:"
