 ...
 * Lucide components; the build step maps `modulePreset` to real feature modules.
 * Reuses the shared `MODULE_PRESETS` so we never duplicate the module catalog.
 */
import { MODULE_PRESETS } from "@hbcfield/shared/client"
...
export interface IndustryMeta {
  ...
  label: string
  /** Lucide icon key (see wizard-icons). */
  icon: string
