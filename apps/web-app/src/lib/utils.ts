...
/*
  Zone-aware wall-clock conversion now lives in `@hbcfield/shared`.

  The mobile attendance editor needs exactly this arithmetic — a shift is
  corrected in the SPACE's zone, not the phone's — and a second copy of a
  DST-aware conversion is the kind of duplicate that drifts silently and by an
  hour. Re-exported here so the existing call sites keep their import.
*/
export { utcToZonedInput, zonedInputToUtc } from "@hbcfield/shared/client"
