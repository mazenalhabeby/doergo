...
const STATUS_BADGES: Record<string, { label: string; className: string; icon: any }> = {
  PENDING_TECHNICIAN: { label: "Awaiting Technician", className: "bg-amber-100 text-amber-700", icon: Clock },
  PENDING_APPROVAL: { label: "Needs Approval", className: "bg-blue-100 text-blue-700", icon: AlertTriangle },
  APPROVED: { label: "Active", className: "bg-emerald-100 text-emerald-700", icon: CheckCircle2 },
  REJECTED: { label: "Rejected", className: "bg-red-100 text-red-700", icon: XCircle },
  EXPIRED_NO_RESPONSE: { label: "Expired", className: "bg-slate-100 text-slate-600", icon: Clock },
  EXPIRED_NO_APPROVAL: { label: "Expired", className: "bg-slate-100 text-slate-600", icon: Clock },
  COMPLETED: { label: "Completed", className: "bg-slate-100 text-slate-700", icon: CheckCircle2 },
  CANCELED: { label: "Declined", className: "bg-slate-100 text-slate-500", icon: XCircle },
}
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between">
            ...
            <Button
              variant="outline"
              size="icon"
              className="h-11 w-11 rounded-xl border-slate-200/80 shadow-sm"
          ...
          <TabsContent value="pending">
            {pendingLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">All overtime requests have been handled</p>
              </div>
            ) : (
          ...
          <TabsContent value="history">
            ...
            {historyLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-lg font-semibold text-slate-700">No overtime history</h3>
              </div>
            ) : (
            ...
            {historyMeta && historyMeta.totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-6">
                ...
                <span className="text-sm text-slate-500">Page {historyPage} of {historyMeta.totalPages}</span>
            ...
            <div className="space-y-4 py-2">
              {approveTarget?.technicianReason && (
                <div className="rounded-lg bg-slate-50 p-3">
                  <p className="text-xs text-slate-500 mb-1">Technician's reason:</p>
                  <p className="text-sm text-slate-700">{approveTarget.technicianReason}</p>
                </div>
              )}
        ...
        <div className="flex items-start gap-4 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            {request.technicianReason && (
              <p className="mt-2 text-sm text-slate-600 bg-slate-50 rounded-md px-3 py-1.5">
                "{request.technicianReason}"
              </p>
            )}
          </div>
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-3 text-xs text-slate-400 mt-0.5">
              ...
              {request.overtimeMinutes && (
                <span className="font-medium text-slate-600">{request.overtimeMinutes} min overtime</span>
              )}
