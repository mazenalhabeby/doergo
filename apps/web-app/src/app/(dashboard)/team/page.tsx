...
export default function TeamPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Teammates in your spaces.</p>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border bg-card py-14 text-center">
          ...
          <p className="text-sm text-muted-foreground">No teammates in your spaces yet.</p>
        </div>
      ) : (
        <div className="grid gap-2 sm:grid-cols-2">
          {colleagues.map((c) => (
            <div key={c.id} className="flex items-center gap-3 rounded-xl border border-border bg-card px-3 py-2.5">
              ...
              <button
                onClick={() => notify.success("Calls coming soon")}
