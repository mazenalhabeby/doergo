...
export default function MyTimeOffPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      notify.success("Time-off requested", "Your manager will review it.")
      setStartDate(""); setEndDate(""); setReason("")
      refetch()
    },
    onError: (e) => notify.error(e instanceof Error ? e.message : "Couldn't submit request"),
  })

  const cancelMut = useMutation({
    mutationFn: (id: string) => employeesApi.cancelTimeOff(id),
    onSuccess: () => { notify.success("Request canceled"); refetch() },
    onError: (e) => notify.error(e instanceof Error ? e.message : "Couldn't cancel"),
  })

  if (!canSee) {
    return (
      <div className="mx-auto max-w-2xl px-6 py-16 text-center text-sm text-muted-foreground">
        You don&apos;t have access to Time Off.
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Request time off and track your requests.</p>
      </div>

      {/* Request form */}
      <div className="rounded-2xl border border-border bg-card p-5 mb-8">
        <h2 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
          <CalendarPlus className="h-4 w-4 text-primary" /> New request
        </h2>
        ...
        <div className="mt-3 flex justify-end">
          <Button size="sm" disabled={!validRange || createMut.isPending} onClick={() => createMut.mutate()}>
            {createMut.isPending ? "Submitting…" : "Request time off"}
          </Button>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-border bg-card py-12 text-center text-sm text-muted-foreground">No time-off requests yet.</div>
      ) : (
            ...
            <div key={r.id} className={`flex items-center gap-4 px-5 py-3 ${i > 0 ? "border-t border-border" : ""}`}>
              <div className="flex-1">
                ...
                {r.status === "REJECTED" && r.rejectionReason && (
                  <div className="text-xs text-red-600">Rejected: {r.rejectionReason}</div>
                )}
              </div>
              ...
              {r.status === "PENDING" && (
                <button onClick={() => cancelMut.mutate(r.id)} disabled={cancelMut.isPending}
                  className="shrink-0 rounded-lg border border-border p-1.5 text-muted-foreground hover:text-red-600" title="Cancel">
