...
export class ShiftIssuesService extends BaseGatewayService {
  ...
  // Writes are sent once: send() retries on a slow answer, and a retried report lands twice.
  create(data: any) { return this.sendOnce({ cmd: 'shift_issue_create' }, data); }
  list(data: any) { return this.send({ cmd: 'shift_issue_list' }, data); }
  get(data: any) { return this.send({ cmd: 'shift_issue_get' }, data); }
  message(data: any) { return this.sendOnce({ cmd: 'shift_issue_message' }, data); }
  acknowledge(data: any) { return this.sendOnce({ cmd: 'shift_issue_acknowledge' }, data); }
  assign(data: any) { return this.sendOnce({ cmd: 'shift_issue_assign' }, data); }
  setStatus(data: any) { return this.sendOnce({ cmd: 'shift_issue_status' }, data); }
