# HBCField — Google Play & App Store Listing (English)

## App Name
HBCField - Field Service Management

## Short Description (80 chars max)
Dispatch tasks, track teams, and manage field operations in real time.

## Full Description (4000 chars max)
HBCField is a powerful field service management platform that connects your office team with technicians in the field. Dispatch tasks, track locations in real time, manage schedules, and close jobs faster — all from one app.

**For Technicians:**
- View and accept assigned tasks instantly
- Navigate to job sites with one tap
- Submit service reports with photos, signatures, and parts used
- Clock in/out with GPS verification
- Track time off and manage your schedule
- Receive push notifications for new assignments

**For Admins & Dispatchers (Web + Mobile):**
- Create and assign tasks to field technicians
- Track technician locations on a live map
- Monitor task progress in real time
- Manage team members, roles, and permissions
- Review service reports and job history
- Manage schedules, time off, and availability
- Multi-language support (English & German)

**Key Features:**
- Real-time GPS tracking and route visualization
- Push notifications for task updates and status changes
- Service reports with before/after photos and digital signatures
- Attendance tracking with geofence verification
- Break management (lunch, short, custom)
- Invitation-based team onboarding
- Organization settings with badge controls
- Dark mode support
- Offline-ready with automatic sync

**Built for Field Service Industries:**
- HVAC & Refrigeration
- Plumbing
- Electrical
- General Contracting
- Cleaning Services
- Security & Maintenance
- IT Field Services

Download HBCField and streamline your field operations today.

## Category
Business / Productivity

## Tags/Keywords
field service, task management, dispatch, technician, GPS tracking, service report, work order, HVAC, plumbing, electrical, field worker, job management, team management

---

## Content Rating Questionnaire Answers

**Violence:** No
**Sexual Content:** No
**Language:** No
**Controlled Substance:** No
**User-Generated Content:** Yes (comments, reports, photos — moderated by org admin)
**Shares Location:** Yes (GPS tracking for technicians during work)
**In-App Purchases:** No
**Ads:** No

**Recommended Rating:** PEGI 3 / Everyone

---

## Target Audience

**Target Age Group:** 18+ (workplace app, not for children)
**Primary Audience:** Field service technicians, dispatchers, and service company owners
**Countries:** Available worldwide (primary: Austria, Germany, DACH region)

---

## Data Safety Form (Google Play)

### Data Collected

| Data Type | Collected | Shared | Purpose |
|-----------|-----------|--------|---------|
| **Email address** | Yes | No | Account creation, login, notifications |
| **Name** | Yes | No | User profile display |
| **Phone number** | No | No | — |
| **Location (precise)** | Yes | No | GPS tracking during work shifts, geofence verification |
| **Location (approximate)** | No | No | — |
| **Photos** | Yes | No | Task attachments, service report documentation |
| **Files/Documents** | Yes | No | Task attachments |
| **App activity** | Yes | No | Task status, clock in/out times |
| **Device ID** | Yes | No | Push notification delivery |
| **Crash logs** | Yes | No | App stability and bug fixes |

### Data Security
- Data encrypted in transit (HTTPS/TLS)
- Data encrypted at rest (database encryption)
- Users can request data deletion
- Data is not sold to third parties

### Data Deletion
- Users can delete their account by contacting their organization admin
- Organization admins can remove members and their data
- Contact: support@hbcfield.com

---

## Privacy Policy URL
https://hbcfield.com/privacy

## Support Email
support@hbcfield.com

## Website
https://hbcfield.com

---

## App Store Connect — Apple-specific fields

These fields exist on Apple but not Google Play. Use them when filling the
App Store listing in App Store Connect → My Apps → HBCField → App Store.

### Subtitle (max 30 characters) — shown under the app name
```
Field service, in real time
```
*(27/30)*

### Promotional Text (max 170 characters) — top of listing, editable without resubmission
```
Dispatch tasks, track teams on a live map, capture service reports with photos & signatures — and close field jobs faster, from one secure app.
```
*(143/170)*

### Keywords (max 100 characters, comma-separated, no spaces after commas)
```
dispatch,technician,jobsite,workorder,scheduling,timesheet,HVAC,plumber,electrician,roofer
```
*(90/100)*

### Marketing URL
https://hbcfield.com

### Support URL
https://hbcfield.com

### Copyright (in App Information)
```
© 2026 HBC GmbH
```

### Demo / review credentials (App Information → App Review Information)
Apple **requires** working credentials for any login-gated app. Create a
dedicated reviewer account on production (do **not** use a personal account)
with the **DISPATCHER** role so the reviewer can see the full task list, live
map, and reports.

```
Username: appreview@hbc-group.eu
Password: <set in production DB; share via App Review Information form>
```

### Notes for the reviewer (App Review Information → Notes)
```
HBCField is a B2B field-service management platform. The reviewer account
above is a DISPATCHER role with sample task and technician data. To exercise
the technician-only flows (clock in/out, complete a task), please contact
us at support@hbc-group.eu — the technician role is mobile-only and tied to
a real worker invitation.
```
